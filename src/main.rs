...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    if let Some(value) = compositor_scene_x11_probe {
        ...
        let config = scene_config_from_cli(
            config::CompositorConfig { visuals: startup_config.visuals, blur_enabled: startup_config.blur_enabled, animation: startup_config.animation, rules: startup_config.rules.clone().into() },
